        ...
        <div v-for="(detail, index) in getCartOrderDetails" :key="index">
          <div>
            <CheckoutCartSummaryItem
              ...
              :item-index="index" />
            <VDivider v-show="index < getCartOrderDetails.length - 1" class="my-5"></VDivider>
          </div>
        </div>
        <VContainer>
          ...
          <VRow align="center" class="black--text">
            <VCol cols="6" class="text-subtitle-1">Subtotal</VCol>
            <VCol cols="6" class="text-right text-subtitle-1 font-weight-bold">
              ${{ (Math.floor(getSubtotalPrice * 100) / 100).toFixed(2) }}
            </VCol>
          </VRow>
        </VContainer>
      </VCardText>
